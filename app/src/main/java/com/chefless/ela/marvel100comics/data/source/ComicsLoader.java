...
/**
 * Custom {@link android.content.Loader} for a list of {@link com.chefless.ela.marvel100comics.data.models.Comic}, using the
 * {@link ComicsRepository} as its source. This Loader is a {@link AsyncTaskLoader} so it queries
 * the data asynchronously.
 */
public class ComicsLoader extends AsyncTaskLoader<List<Comic>>
        implements ComicsRepository.ComicsRepositoryObserver{

    private ComicsRepository mRepository;

    public ComicsLoader(Context context, @NonNull ComicsRepository repository) {
        super(context);
    ...
    protected void onStartLoading() {
        ...
        if (mRepository.cachedComicsAvailable()) {
            deliverResult(mRepository.getCachedComics());
        }
